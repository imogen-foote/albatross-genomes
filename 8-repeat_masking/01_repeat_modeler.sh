...
export LIBDIR=/path/to/repeat_masker/Libraries

# Params
PROJECT=$1 #Antipodean or Gibson's albatross
SAMPLE=$2 #e.g. antipodean_sa52* or gibsons_d36*
FILTER=$3 #e.g. filtered_q8_h50
ASSEMBLY_DIR=/path/to/$PROJECT/data/Chapter3/assemblies/flye/4_nuclear_only/$SAMPLE'_'$FILTER'_nuclear' 
REPEAT_DIR=/path/to/$PROJECT/data/Chapter3/assemblies/flye/5_repeat_masking/$SAMPLE'_'$FILTER'_masked' 
mkdir -p $REPEAT_DIR
...
#extract repeats
singularity exec dfam-tetools-latest.sif famdb.py -i /path/to/repeat_masker/Libraries/RepeatMaskerLib.h5 families -ad --add-reverse-complement Procellariiformes > $REPEAT_DIR/libraries/Procellariiformes_library.fa
